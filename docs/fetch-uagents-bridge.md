...
- Fetch/uAgents/Agentverse/Almanac supply identity/addressing, A2A-facing registration/discovery, mailbox/proxy delivery where needed, hosted-agent surfaces, adapters, and payment protocol primitives.
...
| A2A hosted proof | Operator selects A2A Protocol in Agentverse and supplies the required account/API/endpoint/agent-card or registration setup. |
| Mailbox/proxy transport proof | Operator supplies Agentverse account/API key/seed/mailbox or proxy setup when that transport is needed. |
...
- A2A/Agentverse proof starts with an empty or minimal public tool policy.
...
## Hosted proof direction

For the Teknium-facing proof, choose **A2A Protocol** in Agentverse. Agent Chat Protocol is not the target path for this bridge. Mailbox and Inspector flows remain useful for startup, transport, and no-seed fail-closed checks, but the product story is Hermes local tools reaching Fetch/Agentverse A2A rails through the thinnest safe bridge.
...
- Agentverse external uAgents registration: https://docs.agentverse.ai/documentation/launch-agents/external-agents/u-agents
